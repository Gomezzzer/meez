...
task({ :sample_data => :environment }) do
  ...
  if Rails.env.development?
    ...
    #  recipe_modifier.remote_image_url = Faker::LoremFlickr.image(size: "300x300", search_terms: ['food'])
    #  recipe_modifier.save!
    end

    # Create sample meez en place items for George
    5.times do
      MeezEnPlace.create!(
    ...
    9.times do
      ...
      # Create sample recipe modifiers for each user
      5.times do
        recipe_modifier = RecipeModifier.create!(
          name: Faker::Food.dish,
          description: Faker::Food.description,
      ...
      # Create sample meez en place items for each user
      5.times do
        MeezEnPlace.create!(
          ingredient_name: Faker::Food.ingredient,
          measurement: Faker::Food.measurement,
